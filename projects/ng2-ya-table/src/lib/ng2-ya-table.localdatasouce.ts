import { of } from 'rxjs';
import { DatasourceParameters, TableDataSource } from './ng2-ya-table-interfaces';
...
function getData(item: any, propName: string): string {
    if(propName) {
      return propName.split('.').reduce((prev:any, curr:string) => prev[curr], item);
    }
}
...
export class Ng2YaTableLocalDataSource {
    ...
    toDataSource(): TableDataSource {
        return (request: DatasourceParameters) => {
            ...
            request.filters.filter(c => !!c.value).forEach((column) => {
                filteredData = filteredData.filter((item) => {
