...
describe('DataSourceService', () => {
  ...
  it('#getUsersDataSource with full text filter should return values', () => {
    ...
    service
      .getUsersDataSource({ ...request, fullTextFilter: 'Naruto' })
      .subscribe((r) => (result = r));

    const req = httpTestingController.expectOne(
      `${service['url']}?_page=1&_limit=10&q=Naruto&`
    );
    expect(req.request.method).toEqual('GET');
    req.flush(filtered.data, {
      headers: { 'x-total-count': '' + filtered.recordsTotal }
    });
  ...
  it('#getUsersDataSource with filters should return values', () => {
    ...
    const req = httpTestingController.expectOne(
      `${service['url']}?_page=1&_limit=10&username_like=uzumaki&city_like=gakure&`
    );
    expect(req.request.method).toEqual('GET');
    req.flush(filtered.data, {
      headers: { 'x-total-count': '' + filtered.recordsTotal }
  ...
  it('#getUsersDataSource with orders should return values', () => {
    ...
    const req = httpTestingController.expectOne(
      `${service['url']}?_page=1&_limit=10&_sort=username,city&_order=ASC,DESC&`
    );
    expect(req.request.method).toEqual('GET');
    req.flush(filtered.data, {
      headers: { 'x-total-count': '' + filtered.recordsTotal }
  ...
  it('#getUsersDataSource should not return values', () => {
    ...
    service
      .getUsersDataSource({ ...request, start: 10 })
      .subscribe((r) => (result = r));

    const req = httpTestingController.expectOne(
      `${service['url']}?_page=2&_limit=10&`
    );
    expect(req.request.method).toEqual('GET');
    req.flush(filtered.data, {
      headers: { 'x-total-count': '' + filtered.recordsTotal }
